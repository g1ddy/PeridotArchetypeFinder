# Peridot Archetype MST Tree

```mermaid
flowchart

Jinx007-Jasmine --> Jinx007-Banshee("Jinx007-Banshee (Cheetah)")
Jinx007-Jasmine --> Jinx007-Fuzz("Jinx007-Fuzz (Static)")
Jinx007-Jasmine --> Jinx007-Maximilian
Jinx007-Banshee("Jinx007-Banshee (Cheetah)") --> Jinx007-Chimera("Jinx007-Chimera (Cheetah)")
Jinx007-Banshee("Jinx007-Banshee (Cheetah)") --> Jinx007-Clownking("Jinx007-Clownking (Clownfish,Monkey)")
Jinx007-Fuzz("Jinx007-Fuzz (Static)") --> Jinx007-Lilith("Jinx007-Lilith (Static)")
Jinx007-Fuzz("Jinx007-Fuzz (Static)") --> Jinx007-Turkey("Jinx007-Turkey (Static)")
Jinx007-Maximilian --> Jinx007-Cthulhu
Jinx007-Maximilian --> Jinx007-Neptune
Jinx007-Cthulhu --> Jinx007-Demogorgon
Jinx007-Cthulhu --> Jinx007-Domo("Jinx007-Domo (Elixir)")
Jinx007-Cthulhu --> Jinx007-Dyno
Jinx007-Lilith("Jinx007-Lilith (Static)") --> Jinx007-Dottie("Jinx007-Dottie (Static)")
Jinx007-Lilith("Jinx007-Lilith (Static)") --> Jinx007-Maverick("Jinx007-Maverick (Patina)")
Jinx007-Neptune --> Jinx007-Opal("Jinx007-Opal (Paint)")
Jinx007-Turkey("Jinx007-Turkey (Static)") --> Jinx007-Triumvirate("Jinx007-Triumvirate (Peacock,Psychedelic,Static)")
Jinx007-Turkey("Jinx007-Turkey (Static)") --> Jinx007-Umbra("Jinx007-Umbra (Static)")
Jinx007-Dottie("Jinx007-Dottie (Static)") --> Jinx007-Buggy_Bunny("Jinx007-Buggy Bunny (Clownfish,Rabbit,Static)")
Jinx007-Dottie("Jinx007-Dottie (Static)") --> Jinx007-Euphoria("Jinx007-Euphoria (Clownfish,Unicorn)")
Jinx007-Dyno --> Jinx007-Ethereal("Jinx007-Ethereal (Triton)")
Jinx007-Opal("Jinx007-Opal (Paint)") --> Jinx007-Pinkachu("Jinx007-Pinkachu (Paint)")
Jinx007-Opal("Jinx007-Opal (Paint)") --> Jinx007-Punky
Jinx007-Triumvirate("Jinx007-Triumvirate (Peacock,Psychedelic,Static)") --> Jinx007-Shimmerrosie("Jinx007-Shimmerrosie (Peacock,Static)")
Jinx007-Triumvirate("Jinx007-Triumvirate (Peacock,Psychedelic,Static)") --> Jinx007-Spyro("Jinx007-Spyro (Dragon,Static)")
Jinx007-Triumvirate("Jinx007-Triumvirate (Peacock,Psychedelic,Static)") --> Jinx007-Twilight("Jinx007-Twilight (Static,Sunset)")
Jinx007-Umbra("Jinx007-Umbra (Static)") --> Jinx007-Voltron("Jinx007-Voltron (Static)")
Jinx007-Buggy_Bunny("Jinx007-Buggy Bunny (Clownfish,Rabbit,Static)") --> Jinx007-Aries_II("Jinx007-Aries II (Clownfish,Ram,Static)")
Jinx007-Buggy_Bunny("Jinx007-Buggy Bunny (Clownfish,Rabbit,Static)") --> Jinx007-Burnout
Jinx007-Buggy_Bunny("Jinx007-Buggy Bunny (Clownfish,Rabbit,Static)") --> Jinx007-Fluffish("Jinx007-Fluffish (Cotton Candy,Rabbit)")
Jinx007-Buggy_Bunny("Jinx007-Buggy Bunny (Clownfish,Rabbit,Static)") --> Jinx007-Honeybunny("Jinx007-Honeybunny (Bee,Rabbit)")
Jinx007-Buggy_Bunny("Jinx007-Buggy Bunny (Clownfish,Rabbit,Static)") --> Jinx007-Rocket_Raccoon("Jinx007-Rocket Raccoon (Rabbit,Static)")
Jinx007-Shimmerrosie("Jinx007-Shimmerrosie (Peacock,Static)") --> Jinx007-Fenrir("Jinx007-Fenrir (Static)")
Jinx007-Shimmerrosie("Jinx007-Shimmerrosie (Peacock,Static)") --> Jinx007-Talon("Jinx007-Talon (LED,Rabbit)")
Jinx007-Shimmerrosie("Jinx007-Shimmerrosie (Peacock,Static)") --> Jinx007-Watermelon("Jinx007-Watermelon (Peacock)")
Jinx007-Spyro("Jinx007-Spyro (Dragon,Static)") --> Jinx007-Tulum("Jinx007-Tulum (Static)")
Jinx007-Twilight("Jinx007-Twilight (Static,Sunset)") --> Jinx007-Dark_Blizzard("Jinx007-Dark Blizzard (Anemone,Snowfall)")
Jinx007-Twilight("Jinx007-Twilight (Static,Sunset)") --> Jinx007-Dusk("Jinx007-Dusk (Anemone)")
Jinx007-Twilight("Jinx007-Twilight (Static,Sunset)") --> Jinx007-Hippy("Jinx007-Hippy (Dewdrop)")
Jinx007-Twilight("Jinx007-Twilight (Static,Sunset)") --> Jinx007-Usurper("Jinx007-Usurper (Sunset)")
Jinx007-Voltron("Jinx007-Voltron (Static)") --> Jinx007-Wasted("Jinx007-Wasted (Chameleon,Static)")
Jinx007-Voltron("Jinx007-Voltron (Static)") --> Jinx007-Weaver("Jinx007-Weaver (Static)")
Jinx007-Aries_II("Jinx007-Aries II (Clownfish,Ram,Static)") --> Jinx007-Ghostface("Jinx007-Ghostface (Static,Triceratops)")
Jinx007-Burnout --> Jinx007-Clowndelabra("Jinx007-Clowndelabra (Candle,Clownfish)")
Jinx007-Dark_Blizzard("Jinx007-Dark Blizzard (Anemone,Snowfall)") --> Jinx007-Epsilon("Jinx007-Epsilon (Snowfall)")
Jinx007-Dark_Blizzard("Jinx007-Dark Blizzard (Anemone,Snowfall)") --> Jinx007-Rorschach("Jinx007-Rorschach (Scarab)")
Jinx007-Dark_Blizzard("Jinx007-Dark Blizzard (Anemone,Snowfall)") --> Jinx007-Snowstorm("Jinx007-Snowstorm (Gargoyle)")
Jinx007-Fluffish("Jinx007-Fluffish (Cotton Candy,Rabbit)") --> Jinx007-Faun("Jinx007-Faun (Clownfish)")
Jinx007-Fluffish("Jinx007-Fluffish (Cotton Candy,Rabbit)") --> Jinx007-Gumdrop("Jinx007-Gumdrop (Cotton Candy)")
Jinx007-Hippy("Jinx007-Hippy (Dewdrop)") --> Jinx007-Ingot
Jinx007-Hippy("Jinx007-Hippy (Dewdrop)") --> Jinx007-Jujuba("Jinx007-Jujuba (Dewdrop)")
Jinx007-Honeybunny("Jinx007-Honeybunny (Bee,Rabbit)") --> Jinx007-Golden_Idol("Jinx007-Golden Idol (24Karat,Rabbit)")
Jinx007-Honeybunny("Jinx007-Honeybunny (Bee,Rabbit)") --> Jinx007-Instar("Jinx007-Instar (Bee)")
Jinx007-Tulum("Jinx007-Tulum (Static)") --> Jinx007-Urchin("Jinx007-Urchin (Static)")
Jinx007-Usurper("Jinx007-Usurper (Sunset)") --> Jinx007-Valyrian("Jinx007-Valyrian (Aurora)")
Jinx007-Usurper("Jinx007-Usurper (Sunset)") --> Jinx007-Vici("Jinx007-Vici (Royalty,Sunset)")
Jinx007-Watermelon("Jinx007-Watermelon (Peacock)") --> Jinx007-Chip("Jinx007-Chip (Bismuth,Peacock)")
Jinx007-Watermelon("Jinx007-Watermelon (Peacock)") --> Jinx007-Gallagher("Jinx007-Gallagher (Ram,Watermelon)")
Jinx007-Watermelon("Jinx007-Watermelon (Peacock)") --> Jinx007-Xbreed("Jinx007-Xbreed (Peacock,Watermelon)")
Jinx007-Watermelon("Jinx007-Watermelon (Peacock)") --> Jinx007-Xover("Jinx007-Xover (Peacock)")
Jinx007-Wasted("Jinx007-Wasted (Chameleon,Static)") -...-> New_Archetype:_Cinnabon
Jinx007-Fenrir("Jinx007-Fenrir (Static)") -...-> New_Archetype:_Icicle
Jinx007-Chip("Jinx007-Chip (Bismuth,Peacock)") --> Jinx007-Heiress
Jinx007-Clowndelabra("Jinx007-Clowndelabra (Candle,Clownfish)") --> Jinx007-Dobby("Jinx007-Dobby (Clownfish)")
Jinx007-Clowndelabra("Jinx007-Clowndelabra (Candle,Clownfish)") --> Jinx007-Hotshot("Jinx007-Hotshot (Candle,Clownfish)")
Jinx007-Faun("Jinx007-Faun (Clownfish)") --> Jinx007-Littlefoot("Jinx007-Littlefoot (Yeti)")
Jinx007-Ghostface("Jinx007-Ghostface (Static,Triceratops)") --> Jinx007-Hunter("Jinx007-Hunter (Static)")
Jinx007-Golden_Idol("Jinx007-Golden Idol (24Karat,Rabbit)") --> Jinx007-Helion("Jinx007-Helion (24Karat)")
Jinx007-Gumdrop("Jinx007-Gumdrop (Cotton Candy)") --> Jinx007-Herbelion("Jinx007-Herbelion (Dandelion)")
Jinx007-Gumdrop("Jinx007-Gumdrop (Cotton Candy)") --> Jinx007-Hydrolion("Jinx007-Hydrolion (Clownfish,Dandelion)")
Jinx007-Rorschach("Jinx007-Rorschach (Scarab)") --> Jinx007-Horus
Jinx007-Rorschach("Jinx007-Rorschach (Scarab)") --> Jinx007-Sencha("Jinx007-Sencha (Boba,Glam)")
Jinx007-Rorschach("Jinx007-Rorschach (Scarab)") --> Jinx007-Sunfire("Jinx007-Sunfire (Lobster)")
Jinx007-Snowstorm("Jinx007-Snowstorm (Gargoyle)") --> Jinx007-Galaxy_Gargoyle("Jinx007-Galaxy Gargoyle (Celestial)")
Jinx007-Snowstorm("Jinx007-Snowstorm (Gargoyle)") --> Jinx007-Igor("Jinx007-Igor (Celestial)")
Jinx007-Vici("Jinx007-Vici (Royalty,Sunset)") --> Jinx007-Wreck-It_Ralph("Jinx007-Wreck-It Ralph (Kintsugi,Rabbit)")
Jinx007-Ingot -....-> New_Archetype:_Embers
Jinx007-Galaxy_Gargoyle("Jinx007-Galaxy Gargoyle (Celestial)") --> Jinx007-Interstellar("Jinx007-Interstellar (Celestial)")
Jinx007-Herbelion("Jinx007-Herbelion (Dandelion)") --> Jinx007-Indy
Jinx007-Herbelion("Jinx007-Herbelion (Dandelion)") --> Jinx007-Inflammable("Jinx007-Inflammable (Cotton Candy)")
Jinx007-Herbelion("Jinx007-Herbelion (Dandelion)") --> Jinx007-Inkling("Jinx007-Inkling (Dandelion)")
Jinx007-Horus --> Jinx007-Icarus("Jinx007-Icarus (Beach)")
Jinx007-Igor("Jinx007-Igor (Celestial)") --> Jinx007-Jeronimo("Jinx007-Jeronimo (Gargoyle)")
Jinx007-Sencha("Jinx007-Sencha (Boba,Glam)") --> Jinx007-Thistle("Jinx007-Thistle (Cactus)")
Jinx007-Sencha("Jinx007-Sencha (Boba,Glam)") --> Jinx007-Tumbleweed("Jinx007-Tumbleweed (Boba)")
Jinx007-Sunfire("Jinx007-Sunfire (Lobster)") --> Jinx007-Irukandji
Jinx007-Sunfire("Jinx007-Sunfire (Lobster)") --> Jinx007-Thunderpunch
Jinx007-Wreck-It_Ralph("Jinx007-Wreck-It Ralph (Kintsugi,Rabbit)") --> Jinx007-Xcalibur
Jinx007-Wreck-It_Ralph("Jinx007-Wreck-It Ralph (Kintsugi,Rabbit)") --> Jinx007-Xenoblade("Jinx007-Xenoblade (Kintsugi)")
Jinx007-Irukandji --> Jinx007-Jellybunny("Jinx007-Jellybunny (Jellyfish)")
Jinx007-Thistle("Jinx007-Thistle (Cactus)") --> Jinx007-Umbrella
Jinx007-Thistle("Jinx007-Thistle (Cactus)") --> Jinx007-Unbananable("Jinx007-Unbananable (Banana)")
Jinx007-Tumbleweed("Jinx007-Tumbleweed (Boba)") --> Jinx007-Uva("Jinx007-Uva (Boba)")
Jinx007-Xcalibur --> Jinx007-Yondu("Jinx007-Yondu (Jester)")
Jinx007-Thunderpunch -...-> New_Archetype:_Axolotl
Jinx007-Unbananable("Jinx007-Unbananable (Banana)") --> Jinx007-Werecrow("Jinx007-Werecrow (Patchwork,Scarecrow)")
Jinx007-Werecrow("Jinx007-Werecrow (Patchwork,Scarecrow)") --> Jinx007-Vortex("Jinx007-Vortex (Balloon,Rabbit)")
Jinx007-Vortex("Jinx007-Vortex (Balloon,Rabbit)") --> Jinx007-Wraith("Jinx007-Wraith (Balloon,Rabbit,Static)")
New_Archetype:_Seal -....-> New_Archetype:_Axolotl
New_Archetype:_Vampire -...-> New_Archetype:_Cinnabon
```

